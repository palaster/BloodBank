	...
	public void update() {
		if(!worldObj.isRemote) {
			if(getStackInSlot(0) != null && getStackInSlot(1) != null && getStackInSlot(0).hasTagCompound() && getStackInSlot(1).hasTagCompound()) {
				EntityPlayer predator = BBPlayerHelper.getPlayerFromDimensions(getStackInSlot(0).getTagCompound().getString("PlayerUUID"));
				EntityPlayer prey = BBPlayerHelper.getPlayerFromDimensions(getStackInSlot(1).getTagCompound().getString("PlayerUUID"));
